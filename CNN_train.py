...
class CNN(nn.Module): # CNN 클래스 정의
    def __init__(self):
        super(CNN, self).__init__() # nn.Module 클래스를 상속
...
if __name__ == '__main__': # 이 파일을 직접 실행할때만 True값 리턴
    ...
    data_loader = DataLoader(dataset, batch_size=32, shuffle=True) #pytorch DataLoader 모듈 이용하여 데이터셋 불러오기
    ...
    for epoch in range(1): # 학습 epoch값 설정
        for input, label in tqdm(data_loader): # 데이터 불러오기 (batchsize = 32)
            ...
            optim.zero_grad() #optimizer gradient 초기화
    ...
    weight_dict = {'model_weight': model.state_dict(), 'adam_weight': optim.state_dict()} # 저장시킬 데이터 정의
